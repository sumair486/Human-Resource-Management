...
<div class="row clearfix">
    <div class="col-lg-12">
        <div class="card">
            ...
            <div class="body">
                <form action="{{ url('loan_type') }}" method="post">
                @csrf
                <div class="row clearfix">
                    <div class="col-md-6">
                ...
                <div class="table-responsive">
                    <table id="dtHorizontalExample" class="admin-datatable table table-hover">
                        ...
                        <tbody>
                            ...
                            <tr id="mid1{{$loans->id}}" class="tableReload">
                                <!--<td>-->
                                <!--    <x-options-buttons>-->
                                <!--        <x-slot name="buttons">-->
                                <!--            <li><a href="">Edit</a></li>-->
                                ...
                                <td class="name">{{$loans->name}}</td>
                                <td style="font-size:18px;font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen, Ubuntu, Cantarell, 'Open Sans', 'Helvetica Neue', sans-serif" class="arabic_name">{{$loans->arabic_name}}</td>
                                <td >@if($loans->status == "Active") 
                                    <a href="{{url('loan_type/Inactive',$loans->id)}}"  style="border-radius: 25px 25px;font-size:10px;" class="btn btn-success btn-rounded" type="button">Active</a>
                                @else
                                    <a href="{{url('loan_type/active',$loans->id)}}"  style="border-radius: 25px 25px;font-size:10px;" class="btn btn-danger btn-rounded" type="button">In-active</a>
                                @endif
                                  </td>

                                <td>
                                    <div style="display: flex;">
                                        <a  href="javascript:void(0)"  onclick="editloantype({{$loans->id}})"  class="btn btn-sm btn-default" data-toggle="tooltip" data-placement="top" title="Edit"><i class="far fa-edit"></i></a>

                                        <button type="button" class="btn btn-sm btn-default delete remove" data-id="{{url('loan_type/'.$loans->id)}}" data-toggle="tooltip" data-placement="top" title="Delete"><i class="fa fa-trash"></i></button>
                            ...
                            <!-- Modal -->
                            <div class="modal fade" id="updateloantype" tabindex="-1" aria-labelledby="exampleModalLabel1" aria-hidden="true">
...
<script>
    function editloantype(id){
        $.get('/loan_type/'+id, function(martial){
            $('#id').val(martial.id);
            $('#name').val(martial.name);
            $('.arabic_name').val(martial.arabic_name);

            $('#updateloantype').modal('toggle');
        });
    }

    $('#loantypeEdit').submit(function(e){
        e.preventDefault();
