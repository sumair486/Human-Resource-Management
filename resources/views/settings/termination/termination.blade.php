...
@section('title', 'Termination Type')
@section('page-style')
            ...
            <div class="body">
                <div class="table-responsive">
                    <table id="dtHorizontalExample" class="admin-datatable table table-hover">
                        ...
                        <tbody>
                            @foreach ($termination as $key=>$terminations)
                            <tr id="mid1{{$terminations->id}}" class="tableReload">
                                <!--<td>-->
                                <!--    <x-options-buttons>-->
                                <!--        <x-slot name="buttons">-->
                                <!--            <li><a href="">Edit</a></li>-->
                                ...
                                <td class="name">{{$terminations->name}}</td>
                                <td style="font-size:18px;font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen, Ubuntu, Cantarell, 'Open Sans', 'Helvetica Neue', sans-serif" class="arabic_name">{{$terminations->arabic_name}}</td>
                                <td >@if($terminations->status == "Active") 
                                    <a href="{{url('termination/Inactive',$terminations->id)}}"  style="border-radius: 25px 25px;font-size:10px;" class="btn btn-success btn-rounded" type="button">Active</a>
                                @else
                                    <a href="{{url('termination/active',$terminations->id)}}"  style="border-radius: 25px 25px;font-size:10px;" class="btn btn-danger btn-rounded" type="button">In-active</a>
                                @endif
                                  </td>

                                <td>
                                    <div style="display: flex;">
                                        <a  href="javascript:void(0)"  onclick="editterminationtype({{$terminations->id}})"  class="btn btn-sm btn-default" data-toggle="tooltip" data-placement="top" title="Edit"><i class="far fa-edit"></i></a>

                                        <button type="button" class="btn btn-sm btn-default delete remove" data-id="{{url('termination/'.$terminations->id)}}" data-toggle="tooltip" data-placement="top" title="Delete"><i class="fa fa-trash"></i></button>
                            ...
                            <!-- Modal -->
                            <div class="modal fade" id="updateterminationtype" tabindex="-1" aria-labelledby="exampleModalLabel1" aria-hidden="true">
...
<script>
    function editterminationtype(id){
        $.get('/termination/'+id, function(martial){
            $('#id').val(martial.id);
            $('#name').val(martial.name);
            $('.arabic_name').val(martial.arabic_name);

            $('#updateterminationtype').modal('toggle');
        });
    }

    $('#terminationtypeEdit').submit(function(e){
        e.preventDefault();
