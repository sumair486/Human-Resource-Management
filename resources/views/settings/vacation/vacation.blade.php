...
<div class="row clearfix">
    <div class="col-lg-12">
        <div class="card">
            ...
            <div class="body">
                <form action="{{ url('vacation_type') }}" method="post">
                @csrf
                <div class="row clearfix">
                    <div class="col-md-6">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            <label >Name</label>
                            <input   dir="rtl"  id="text1" style="text-align:right;" type="text"  placeholder="In Arabic" name="arabic_name" class="form-control form-control-sm" value="{{old('arabic_name')}}">
                            @error('arabic_name')
                        ...
                        <tbody>
                            ...
                            <tr id="mid1{{$vacations->id}}" class="tableReload">
                                ...
                                <td >@if($vacations->status == "Active") 
                                    <a href="{{url('vacation_type/Inactive',$vacations->id)}}"  style="border-radius: 25px 25px;font-size:10px;" class="btn btn-success btn-rounded" type="button">Active</a>
                                @else
                                    <a href="{{url('vacation_type/active',$vacations->id)}}"  style="border-radius: 25px 25px;font-size:10px;" class="btn btn-danger btn-rounded" type="button">In-active</a>
                                @endif
                                  </td>

                                <td>
                                    <div style="display: flex;">
                                        <a  href="javascript:void(0)"  onclick="editvacationtype({{$vacations->id}})"  class="btn btn-sm btn-default" data-toggle="tooltip" data-placement="top" title="Edit"><i class="far fa-edit"></i></a>

                                        <button type="button" class="btn btn-sm btn-default delete remove" data-id="{{url('vacation_type/'.$vacations->id)}}" data-toggle="tooltip" data-placement="top" title="Delete"><i class="fa fa-trash"></i></button>
                            ...
                            <!-- Modal -->
                            <div class="modal fade" id="updatevacationtype" tabindex="-1" aria-labelledby="exampleModalLabel1" aria-hidden="true">
...
@push('after-scripts')
<script>
    function editvacationtype(id){
        $.get('/vacation_type/'+id, function(martial){
            $('#id').val(martial.id);
            $('#name').val(martial.name);
            $('.arabic_name').val(martial.arabic_name);

            $('#updatevacationtype').modal('toggle');
        });
    }

    $('#vacationtypeEdit').submit(function(e){
        e.preventDefault();
