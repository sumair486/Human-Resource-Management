                ...
                <div class="table-responsive">
                    <table id="dtHorizontalExample" class="admin-datatable table table-hover">
                        ...
                        <tbody>
                            ...
                            <tr id="mid1{{$qualifications->id}}" class="tableReload">
                                <!--<td>-->
                                <!--    <x-options-buttons>-->
                                <!--        <x-slot name="buttons">-->
                                <!--            <li><a href="">Edit</a></li>-->
                                ...
                                <td class="name">{{$qualifications->name}}</td>
                                <td style="font-size:18px;font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen, Ubuntu, Cantarell, 'Open Sans', 'Helvetica Neue', sans-serif" class="arabic_name">{{$qualifications->arabic_name}}</td>
                                <td >@if($qualifications->status == "Active") 
                                    <a href="{{url('qualification/Inactive',$qualifications->id)}}"  style="border-radius: 25px 25px;font-size:10px;" class="btn btn-success btn-rounded" type="button">Active</a>
                                @else
                                    <a href="{{url('qualification/active',$qualifications->id)}}"  style="border-radius: 25px 25px;font-size:10px;" class="btn btn-danger btn-rounded" type="button">In-active</a>
                                @endif
                                  </td>

                                <td>
                                    <div style="display: flex;">
                                        <a  href="javascript:void(0)"  onclick="editQualification({{$qualifications->id}})"  class="btn btn-sm btn-default" data-toggle="tooltip" data-placement="top" title="Edit"><i class="far fa-edit"></i></a>

                                        <button type="button" class="btn btn-sm btn-default delete remove" data-id="{{url('qualification/'.$qualifications->id)}}" data-toggle="tooltip" data-placement="top" title="Delete"><i class="fa fa-trash"></i></button>
                            ...
                            <!-- Modal -->
                            <div class="modal fade" id="updateQualification" tabindex="-1" aria-labelledby="exampleModalLabel1" aria-hidden="true">
...
<script>
    function editQualification(id){
        $.get('/qualification/'+id, function(qualification){
            $('#id').val(qualification.id);
            $('#name').val(qualification.name);
            $('.arabic_name').val(qualification.arabic_name);

            $('#updateQualification').modal('toggle');
        });
    }

    $('#qualificationEdit').submit(function(e){
        e.preventDefault();
