...
<div class="row clearfix">
    <div class="col-lg-12">
        <div class="card">
            ...
            <div class="body">
                <form action="{{ url('nationality') }}" method="post">
                @csrf
                <div class="row clearfix">
                    <div class="col-md-6">
            ...
            <div class="body">
                <div class="table-responsive">
                    <table id="dtHorizontalExample" class="admin-datatable table table-hover">
                        ...
                        <tbody>
                            @foreach ($nationality as $key=>$nationalities)
                            <tr id="mid1{{$nationalities->id}}" class="tableReload">
                                <!--<td>-->
                                <!--    <x-options-buttons>-->
                                <!--        <x-slot name="buttons">-->
                                <!--            <li><a href="">Edit</a></li>-->
                                ...
                                <!--</td>-->
                                <td>{{ $key+1 }}</td>
                                <td class="name">{{$nationalities->name}}</td>
                                <td style="font-size:18px;font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen, Ubuntu, Cantarell, 'Open Sans', 'Helvetica Neue', sans-serif" class="arabic_name">{{$nationalities->arabic_name}}</td>
                                <td >@if($nationalities->status == "Active") 
                                    <a href="{{url('nationality/Inactive',$nationalities->id)}}"  style="border-radius: 25px 25px;font-size:10px;" class="btn btn-success btn-rounded" type="button">Active</a>
                                @else
                                    <a href="{{url('nationality/active',$nationalities->id)}}"  style="border-radius: 25px 25px;font-size:10px;" class="btn btn-danger btn-rounded" type="button">In-active</a>
                                @endif
                                  </td>

                                <td>
                                    <div style="display: flex;">
                                        <a  href="javascript:void(0)"  onclick="editNationality({{$nationalities->id}})"  class="btn btn-sm btn-default" data-toggle="tooltip" data-placement="top" title="Edit"><i class="far fa-edit"></i></a>

                                        <button type="button" class="btn btn-sm btn-default delete remove" data-id="{{url('nationality/'.$nationalities->id)}}" data-toggle="tooltip" data-placement="top" title="Delete"><i class="fa fa-trash"></i></button>
                            ...
                            <!-- Modal -->
                            <div class="modal fade" id="updateNationality" tabindex="-1" aria-labelledby="exampleModalLabel1" aria-hidden="true">
...
<script>
    function editNationality(id){
        $.get('/nationality/'+id, function(nationality){
            $('#id').val(nationality.id);
            $('#name').val(nationality.name);
            $('.arabic_name').val(nationality.arabic_name);

            $('#updateNationality').modal('toggle');
        });
    }

    $('#nationalityEdit').submit(function(e){
        e.preventDefault();
